...
@router.get("/")
async def read_depositos(request: Request, db: Session = Depends(get_database_session)): #funcion para leer todos los depositos
...
async def create_deposito(request: Request, db: Session = Depends(get_database_session)):
...
async def create_deposito(db: Session = Depends(get_database_session), descripcion = Form(...)): #al agregar una variable = fomrulario (...) lo vuelve obligatorio
...
async def listar_depositos(request: Request, db: Session = Depends(get_database_session)): 
...
def editar(db: Session = Depends(get_database_session), iddeposito = Form(...), descripcion = Form(...)): #los names dentro del .html deben llamarse igual que los parametros de esta funcion
