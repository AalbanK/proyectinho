...
async def read_cliente(request: Request, db: Session = Depends(get_database_session)):
    ...
    return templates.TemplateResponse("clientes/listar.html", {"request": request, "data": records})
...
@router.post("/nuevo")
async def create_cliente(db: Session = Depends(get_database_session), descCliente = Form(...), idDepartamento=Form(...)):
    cliente = Cliente(descripcion=descCliente, id_departamento=idDepartamento)
    db.add(cliente)
    db.commit()
    db.refresh(cliente)
