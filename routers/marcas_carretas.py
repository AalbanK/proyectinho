...
async def read_marca_carreta(request: Request, db: Session = Depends(get_database_session)):#.TemplateResponse muestra la interfaz (html)
...
async def create_marca_carreta(db: Session = Depends(get_database_session), marca_carre_desc = Form(...)):#al agregar una variable = fomrulario (...) lo vuelve obligatorio
...
async def listar_marcascarr(request: Request, db: Session = Depends(get_database_session)): 
...
def editar(db: Session = Depends(get_database_session), idmarca_carreta = Form(...), descripcion = Form(...)): #los names dentro del .html deben llamarse igual que los parametros de esta funcion
