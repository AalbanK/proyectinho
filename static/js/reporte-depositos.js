...
const urlInformeStock = '/reportes/depositos';
const titulo_reporte = 'Movimientos de Depósitos'
...
inicializarDataTable = (tabla, datos, columnas, urlIdioma, parametros, titulo_reporte) => {
  ...
  return tabla.DataTable({
    ...
    buttons: [
      ...
      {
        extend: 'pdfHtml5', // extensión del plugin
...
$(document).ready(function(){
    ...
    tablaStock = $("#movimientos_depositos");
    ...
    document.querySelector('#parametros_depositos').addEventListener('submit', function(e) {
        ...
        let deposito = document.querySelector('#inputDeposito');
        ...
        parametros_dt.Deposito = parametros.deposito ? (deposito.options[deposito.selectedIndex].text) : 'Todos' // Muestra el texto "Todos" si no se pasó el parámetro, caso contrario muestra el texto de la opción seleccionada en el select
        ...
        .then(json => {
            datos = json;
            columnas = [
                { title: "Fecha", data: "fecha", render: function(data, type, row){
                ...
                { title: "Nro. Documento", data: "numero_doc" },
                { title: "Producto", data: "descripcion" },
                { title: "Cantidad", data: "cantidad" },
                
                { title: "Deposito Origen", data: "deposito_origen" },
                { title: "Deposito Destino", data: "deposito_destino" }
            ]
